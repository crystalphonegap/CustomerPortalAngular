...
@Component({
  selector: 'app-saporder-list',
  templateUrl: './saporder-list.component.html',
  styleUrls: ['./saporder-list.component.css']
})
export class CFSAPOrderListComponent implements OnInit {
  Orders: any=[];
  constructor(private _CFAgentService:CFAgentService, public datepipe: DatePipe,private _OrderServiceService: OrderService, private service: UserService, private router: Router, private _CustomerService: CustomerService
...
this.loadedFromDate=false
  ...
  Search(){
    ...
    if(this.UserTypetxt==UserConstant.CFAgent){
  ...
  getAllOrders(pageNo) {
    this._CfAgentComponent.setLoading(true);
   this,pageNo=pageNo,
    this._OrderServiceService.GetAllOrdersByCFCode(this.FromDate,this.Todate,'Posted To SAP',UserConstant.CFAgent, this.UserCode, this.pageNo, this.OrdersPerPage, this.search).subscribe((data: any) => {
      this.Orders = data ;
      this._CfAgentComponent.setLoading(false);
  ...
  pass(value): void {
    ...
    this.router.navigateByUrl('/CfAgent/SAPOrderView');
  }
  ...
  download() {
    ...
    this._CfAgentComponent.setLoading(true);
    this._CFAgentService.ExportToExcel(this.FromDate,this.Todate,'Posted To SAP',UserConstant.CFAgent,this.UserCode, this.search).subscribe(response => {
			let blob:any = new Blob([response], { type: 'text/json; charset=utf-8' });
			const url = window.URL.createObjectURL(blob);
      this._CfAgentComponent.setLoading(false);
			fileSaver.saveAs(blob, 'PostedToSAPOrderList.xlsx');
		},
    err => { 
      this._CfAgentComponent.setLoading(false);
        console.log(err);
