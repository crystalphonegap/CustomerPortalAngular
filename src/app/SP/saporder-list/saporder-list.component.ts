...
export class SPSAPOrderListComponent implements OnInit {
  Orders: any= [];
...
this.loadedFromDate=false
  ...
  getAllOrders(pageNo) {
    this._SPComponent.setLoading(true);
   this.pageNo=pageNo;
    this._OrderServiceService.GetAllOrdersByCFCode(this.FromDate,this.Todate,'Posted To SAP',this.UserTypetxt,this.UserCode, this.pageNo, this.OrdersPerPage, this.search).subscribe((data: any) => {
      this.Orders = data ;
      this._SPComponent.setLoading(false);
  ...
  pass(value): void {
    ...
    this.router.navigateByUrl('/SP/SAPOrderView');
  }
  ...
  download() {
    ...
    this._SPComponent.setLoading(true);
    this._CFAgentService.ExportToExcel(this.FromDate,this.Todate,'Posted To SAP',this.UserTypetxt,this.UserCode, this.search).subscribe(response => {
