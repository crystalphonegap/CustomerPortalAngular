...
export class TicketMisListForUsertypeComponent implements OnInit {
  Tickets: any =[];
  constructor(public datepipe: DatePipe, private _EmpComponent : EmpComponent ,   private _TicketService: TicketService, private service: UserService, private router: Router, private _CustomerService: CustomerService
...
this.loadedFromDate=false
  ...
  pass(value,status,Edit): void {
    ...
    if(Edit == 0){
      ...
      this.router.navigateByUrl('/Emp/MisReportViewOpenTicketByUserType');
      else{
        this.router.navigateByUrl('/Emp/MisReportViewCloseTicketByUserType');
      }
    }
    else{
      this.router.navigateByUrl('/Emp/MisReportViewCloseTicketByUserType');
