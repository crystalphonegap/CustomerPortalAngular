...
export class TechnicianOpenTicketsComponent implements OnInit {
  Tickets: any =[];
...
this.loadedFromDate=false
  ...
  getClosed() {
    this._EmpComponent.setLoading(true);
    this._TicketService.GetTicketCount(this.FromDate, this.Todate, localStorage.getItem('UserCode'),localStorage.getItem('UserType'), 'Closed',this.search).subscribe((res: any) => {
      this.Closed = res;
      this._EmpComponent.setLoading(false);
      if (res == "" || res == null) {
        this.Closed = 0;
  ...
  pass(value,status,Edit): void {
    ...
    if(Edit == 0 || Edit ==2){
      ...
      this.router.navigateByUrl('/Emp/OpenTicketsView');
      else{
        this.router.navigateByUrl('/Emp/ClosedTicketsView');
      }
    }
    else{
      this.router.navigateByUrl('/Emp/ClosedTicketsView');
