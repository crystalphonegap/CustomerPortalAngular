...
@Component({
  selector: 'app-rm-upload',
  templateUrl: './rm-upload.component.html',
  styleUrls: ['./rm-upload.component.css']
})
export class SystemAdminRmUploadComponent implements OnInit {
  ...
  Submit(){
    ...
    this._SystemAdminService.uploadRMExcelData( formData)
      .subscribe(
        (res: any) => {
  ...
  download() {
    this._SystemAdminComponent.setLoading(true);
    this._SystemAdminService.DownloadSampleForEmployeeUpload("Regional Manager").subscribe(response => {
			let blob:any = new Blob([response], { type: 'text/json; charset=utf-8' });
			const url = window.URL.createObjectURL(blob);
      this._SystemAdminComponent.setLoading(false);
			fileSaver.saveAs(blob, 'SampleRegionalManagerUpload.xlsx');
		},
    err => {
      this._SystemAdminComponent.setLoading(false);
  ...
  back()
  {
    this.router.navigateByUrl('/SystemAdmin/RMUpload');
  }
