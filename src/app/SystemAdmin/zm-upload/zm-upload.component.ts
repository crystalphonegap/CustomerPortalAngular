...
@Component({
  selector: 'app-zm-upload',
  templateUrl: './zm-upload.component.html',
  styleUrls: ['./zm-upload.component.css']
})
export class SystemAdminZmUploadComponent implements OnInit {
...
Submit(){
  ...
  this._SystemAdminService.uploadZMExcelData( formData)
    .subscribe(
      (res: any) => {
      ...
      err => {
        ...
        if(error=='file is  uploaded Successfully.' ){
          ...
          this.alertService.error(error);
          this._SystemAdminService.DownloadErrorFileforEmployeeUpload("Zonal Manager").subscribe(response => {
...
download() {
  this._SystemAdminComponent.setLoading(true);
  this._SystemAdminService.DownloadSampleForEmployeeUpload("Zonal Manager").subscribe(response => {
    let blob:any = new Blob([response], { type: 'text/json; charset=utf-8' });
    this._SystemAdminComponent.setLoading(false);
    const url = window.URL.createObjectURL(blob);
    fileSaver.saveAs(blob, 'SampleZMUpload.xlsx');
  },
  err => {
    this._SystemAdminComponent.setLoading(false);
    console.log('Error downloading the file');
