...
@Component({
  selector: 'app-ticket-mis-list-for-usertype',
  templateUrl: './ticket-mis-list-for-usertype.component.html',
  styleUrls: ['./ticket-mis-list-for-usertype.component.css']
})
export class TicketMisListForUsertypeComponent implements OnInit {
  Tickets: any =[];
  constructor(public datepipe: DatePipe, private _SystemAdminComponent : SystemAdminComponent ,   private _TicketService: TicketService, private service: UserService, private router: Router, private _CustomerService: CustomerService
...
this.loadedFromDate=false
  ...
  ngOnInit() {
     ...
     this.userType=this.MISDATA.userType;
     this.Type=this.MISDATA.Type;
  ...
  getAllTickets(PageNo) {
    ...
    this._TicketService.getTicketMISListByUsertype(localStorage.getItem('UserCode'),localStorage.getItem('UserType'),
    this.Priority,this.Type,this.userType,this.From,this.To,
  ...
  getAllTicketsCount() {
    ...
    this._SystemAdminComponent.setLoading(true);
    this._TicketService.getTicketMISListCountByUsertype(
      localStorage.getItem('UserCode'),localStorage.getItem('UserType')
    ,this.Priority,this.Type,this.userType,this.From,this.To,this.search).subscribe((res: any) => {
  ...
  pass(value,status,Edit): void {
    ...
    this.router.navigateByUrl('/SystemAdmin/MisReportViewOpenTicketByUserType');
    // if(Edit == 0){
    //     if(status=='Open' ||status=='In-Process'  )
    //   this.router.navigateByUrl('/SystemAdmin/MisReportViewOpenTicketByUserType');
    //   else{
    //     this.router.navigateByUrl('/SystemAdmin/MisReportViewCloseTicketByUserType');
    //   }
    // }
    // else{
    //   this.router.navigateByUrl('/SystemAdmin/MisReportViewCloseTicketByUserType');
  ...
  download() {
    ...
    this._TicketService.DownloadMISReportAssignToWiseMIS(localStorage.getItem('UserCode'),localStorage.getItem('UserType')
   ,this.Priority,this.Type,this.userType,this.From,this.To,this.search).subscribe(response => {
			let blob:any = new Blob([response], { type: 'text/json; charset=utf-8' });
      const url = window.URL.createObjectURL(blob);
    this._SystemAdminComponent.setLoading(false);

			fileSaver.saveAs(blob, 'TicketMISReportUserTypeWiseList.xlsx');
    },
    err => {
      this._SystemAdminComponent.setLoading(false);
      console.log(err);
