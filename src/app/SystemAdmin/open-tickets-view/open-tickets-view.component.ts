...
@Component({
  selector: 'app-open-tickets-view',
  templateUrl: './open-tickets-view.component.html',
  styleUrls: ['./open-tickets-view.component.css']
})
export class TechnicianOpenTicketsViewComponent implements OnInit {
  ...
  getTicketInfo() {
    ...
    this._TicketService.getTicketHeaderDetail(this.TicketId).subscribe((res: any) => {
      ...
      if (this.TicketInfo.Statusvtxt == "Open") {
        ...
        this.Resolved = true;
        this.Close = false;
      } else if (this.TicketInfo.Statusvtxt == "Closed") {
        this.Open = false;
        this.InProcess = false;
        this.Resolved = false;
        this.Close = true;
  ...
  UploadAttachment(ID){
    ...
    this._TicketService.UploadFileForEmp( formData,ID,this.TicketInfo.RefNovtxt,localStorage.getItem(constStorage.UserCode))
      .subscribe(
        (res: any) => {
          this._SystemAdminComponent.setLoading(false);
          this.router.navigateByUrl('/SystemAdmin/TicketList');
